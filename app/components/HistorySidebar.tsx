...
export const HistorySidebar: React.FC<HistorySidebarProps> = ({ isOpen, onClose }) => {
    ...
    <div
      ...
      className={`p-4 rounded-lg transition-colors cursor-pointer border ${
        selectedPlanId === plan.id
          ? 'bg-georgianRed/20 border-georgianRed'
          : 'bg-white/5 border-white/10 hover:bg-white/10'
      }`}
    >
      <div className="flex justify-between items-center mb-2">
        <p className="font-bold text-white truncate w-48">{plan.destination}</p>
        {plan.status === 'generating' && <Loader2 className="animate-spin text-blue-400" size={18} />}
        {plan.status === 'error' && <ServerCrash className="text-red-400" size={18} />}
        {plan.status === 'completed' && <Sparkles className="text-green-400" size={18} />}
      </div>
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-40 animate-in fade-in-0" onClick={onClose}>
      <div 
        className="fixed top-0 right-0 h-full w-full max-w-2xl bg-gray-900/95 border-l border-white/10 shadow-2xl z-50 flex flex-col animate-in slide-in-from-right-10 duration-300"
       ...
       >
        ...
        <div className="flex-grow flex overflow-hidden">
          <div className="w-1/3 border-r border-white/10 overflow-y-auto p-4 space-y-3">
          ...
          <div className="w-2/3 overflow-y-auto p-6">
            {selectedPlan ? (
              <article className="prose prose-sm md:prose-base prose-invert max-w-none text-gray-300">
                <h2 className="text-2xl font-bold text-white mb-4 !mt-0">{selectedPlan.destination}</h2>
                ...
                {selectedPlan.status === 'error' && <p className="text-red-400">Error: {selectedPlan.result || 'An unknown error occurred.'}</p>}
              </article>
            ) : (
